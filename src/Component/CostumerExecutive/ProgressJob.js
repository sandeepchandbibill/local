...
class ProgressJobs extends Component {
  ...
  getSellerData = ()=>{
    let headers = {
     ...
     console.log(headers)
    axios.get('http://192.168.1.62:4000/api/jobs/progress',{ headers })
  ...
  editSeller = (items) =>{
    ...
    this.props.history.push({pathname: '/payoutedit/',
    state:{seller_id: items }
    })
  }
  render() {
              ...
              <CardBody>
                <Table className="able" hover bordered striped responsive size="sm">
                  ...
                  <tr key={items.id} className="myList">
                    ...
                    <td><FontAwesomeIcon className="mt-3" onClick={()=>this.editSeller(items.id)}  icon= {faEdit} color="blue" /></td>
                  </tr>
                  </tbody>
                  )
                }
